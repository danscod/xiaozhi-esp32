        ...
        switch (state_) {
          case ParseState::FIND_PAGE: {
            ...
            if (ctx_.bytes_needed < 4) {
                ...
                ESP_LOGE(TAG, "OggDemuxer run in error state: bytes_needed=%lu", (unsigned long)ctx_.bytes_needed);
                Reset();
                return processed;
            }
            break;
          ...
          case ParseState::PARSE_DATA: {
            while (ctx_.seg_index < ctx_.seg_count && processed < size) {
                ...
                if (ctx_.packet_len + seg_len > sizeof(ctx_.packet_buf)) {
                    ESP_LOGE(TAG, "包缓冲区溢出: %lu + %u > %lu", (unsigned long)ctx_.packet_len, seg_len, (unsigned long)sizeof(ctx_.packet_buf));
                    state_ = ParseState::FIND_PAGE;
                    ctx_.packet_len = 0;
                    ctx_.packet_continued = false;
                    ctx_.seg_remaining = 0;
            ...
            if (ctx_.seg_index == ctx_.seg_count) {
                ...
                if (ctx_.body_offset < ctx_.body_size) {
                    ESP_LOGW(TAG, "数据体不完整: %lu/%lu",
                            (unsigned long)ctx_.body_offset, (unsigned long)ctx_.body_size);
                }
